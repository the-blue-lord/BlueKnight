module.exports = (variables_string, placeholders_string, starting_delimiter = "<!--", ending_delimiter = "--!>") => {
    ...
    const additional_padding = "padding";

    const newPlaceholders = (placeholders_string.slice(startingIndex+starting_delimiter.length)+additional_padding).toString();
    let newVariables = variables_string.slice(startingIndex)+additional_padding;
    newVariables = newVariables.toString();
